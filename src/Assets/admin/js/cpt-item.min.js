...
var CodeEditor = /*#__PURE__*/function () {
  ...
  _createClass(CodeEditor, [{
    ...
    value: function setupEditor(instanceSettings, langOptions) {
      ...
      this.editor._langOptions = langOptions;
      var options = this.getOptions(langOptions);
      this.editor.setOptions(options);
      this.syncTextareaAndEditor(langOptions);
    ...
    key: "getOptions",
    value: function getOptions(langOptions) {
      // for CSS it's buggy, so keep it available by Ctrl + Space only.
      var isLiveAutocompletionEnabled = '_css' !== langOptions.mode;
      return {
        theme: "ace/theme/nord_dark",
        value: this.getInputValue(langOptions),
    ...
    key: "getInputValue",
    value: function getInputValue(langOptions) {
      var value = this.options.textarea.value;
      return this.options.enginesStorage.unpackTemplate(value, langOptions.engine);
    }
  }, {
    key: "setInputValue",
    value: function setInputValue(value, langOptions) {
      this.options.textarea.value = this.options.enginesStorage.packTemplate(value, langOptions.engine);
    }
  }, {
    key: "syncTextareaAndEditor",
    value: function syncTextareaAndEditor(langOptions) {
      var _this2 = this;
      // to avoid recursion
      var isExternalEvent = false;
      this.editor.getSession().on('change', function () {
        var value = _this2.editor.getSession().getValue();
        _this2.setInputValue(value, langOptions);
        isExternalEvent = true;
        // notify potential listeners, e.g. fields-changed notice
        _this2.options.textarea.dispatchEvent(new Event('change'));
        isExternalEvent = false;
      });
      // update, if the value was reloaded
      this.options.textarea.addEventListener('change', function () {
        if (isExternalEvent) {
          return;
        }
        var value = _this2.getInputValue(langOptions);
        _this2.editor.getSession().setValue(value);
        // when editor is visible, setValue automatically redraws it
        if (_this2.options.tab.classList.contains('active')) {
          return;
        }
        ...
        _this2.options.tab.addEventListener('click', resize);
      });
    }
  }, {
    ...
    value: function registerCompleter(langTools) {
      ...
      langTools.addCompleter({
        getCompletions: function getCompletions(editor, session, position, prefix, callback) {
          ...
          var interpreter = _this3.options.enginesStorage.resolveInterpreter(langOptions.engine);
          if (interpreter) {
...
var CodeField = /*#__PURE__*/function (_MyElement) {
  ...
  _createClass(CodeField, [{
    ...
    value: function make() {
      ...
      var modeSettings = JSON.parse(JSON.stringify(_settings__WEBPACK_IMPORTED_MODULE_1__["default"]['mods']));
      ...
      var codeEditor = new _code_editor__WEBPACK_IMPORTED_MODULE_2__.CodeEditor({
      ...
      codeEditor.setupEditor(modeSettings[this.langOptions.mode], this.langOptions);
...
var RefreshRequest = /*#__PURE__*/function () {
  function RefreshRequest(enginesStorage, textareas, refreshRoute, refreshNonce) {
  ...
  _createClass(RefreshRequest, [{
    ...
    value: function reload(acfFields, data) {
      ...
      this.refreshTextareas(data['textareaItems']);
    ...
    value: function refreshTextareas(items) {
      for (var _i = 0, _Object$entries = Object.entries(items); _i < _Object$entries.length; _i++) {
        var _Object$entries$_i = _slicedToArray(_Object$entries[_i], 2),
          textareaId = _Object$entries$_i[0],
          value = _Object$entries$_i[1];
        var textarea = this.resolveTextareaElement(textareaId);
        if (textarea) {
    ...
    value: function makeRequest() {
      ...
      fetch('/wp-json/acf_views/v1' + this.refreshRoute, {
        ...
        _this.reload(acfFields, response);
  ...
  _createClass(PostBox, [{
    ...
    value: function createCodeFields() {
      ...
      if (!_settings__WEBPACK_IMPORTED_MODULE_3__["default"].hasOwnProperty('markupTextarea')) {
        new Error('Required argument is missing: markupTextarea');
      }
    ...
    * we should avoid this. In addition, even if users enable the fullscreen mode, it should be kept among reloads
    ...
    value: function setupRefreshAjax() {
      ...
      new _interactive_fields_refresh_request__WEBPACK_IMPORTED_MODULE_6__.RefreshRequest(this.enginesStorage, textareaItems, refreshRoute, refreshNonce);
    }
  }, {
    key: "init",
    value: function init() {
