...
final class Layout_Interactive_Fields extends Cpt_Interactive_Fields {
	...
	protected function get_interactive_response( WP_Post $post ): array {
		...
		return array(
			'textareaItems'    => $this->get_textarea_items( $layout_settings ),
			'elements'         => $this->get_html_elements( $post, $layout_settings ),
			'autocompleteData' => $this->layout_factory->get_autocomplete_variables( $layout_unique_id ),
		);
	}

	protected function get_textarea_items( Layout_Settings $layout_settings ): array {
		ob_start();
	 ...
	 */
	protected function get_html_elements( WP_Post $post, Layout_Settings $layout_settings ): array {
		ob_start();
